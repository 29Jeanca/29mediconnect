...
export default function StepAccountDetails({ onBack, userType }) {
  const [data, setData] = useState({
    usuario: '', contrasena: '', repetir: '', alias: '', usuarioProfesional: ''
  });
  ...
  const handleSubmit = async() => {
    ...
    if (userType === 'paciente') {
      ...
      if (pacienteInfo.usuario && pacienteInfo.alias && data.contrasena && data.repetir) {
        if (data.contrasena === data.repetir) {
  ...
  return (
    <Stack spacing={2}>
      {userType === 'doctor' && (
        <TextField label="Nombre de usuario profesional" name="usuarioProfesional" value={data.usuarioProfesional} onChange={(e)=> setDocInfo({ ...docInfo,usuarioProfesional: e.target.value })} />
      )}
      {userType === 'paciente' && (
             
        <>
          <TextField label="Nombre de usuario" name="usuario" value={pacienteInfo.usuario} onChange={(e)=> setPacienteInfo({ ...pacienteInfo, usuario: e.target.value })}/>
          <TextField label="Alias o nombre preferido" name="alias" value={pacienteInfo.alias} onChange={(e)=> setPacienteInfo({ ...pacienteInfo, alias: e.target.value })} />
        </>
      )}
